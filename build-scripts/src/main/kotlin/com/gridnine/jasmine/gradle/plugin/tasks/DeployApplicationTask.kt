...
open class DeployApplicationTask: DefaultTask() {
    ...
    fun execute() {
        ...
        for (file in project.file("build/dist/lib").listFiles()?: emptyArray()) {
            if (file.isFile) {
    ...
    private fun makeRequest(command: String, params: String? = null, body: ByteArray? = null):ByteArray{
        ...
        Socket(inetAddress, port).use { socket ->
            ...
            socket.getOutputStream().use{ out ->
                out.write(byteArrayOf(command.length.toByte(), params?.length?.toByte() ?: 0.toByte()))
    ...
    private fun calculateCheckSum(md: MessageDigest,
                                  strm: ZipInputStream) {
        while (true) {
            val entry: ZipEntry = strm.nextEntry ?: break

...
import com.gridnine.jasmine.server.core.storage.jdbc.DatabaseCollectionDescription
import com.gridnine.jasmine.server.core.storage.jdbc.DatabasePropertyDescription
import com.gridnine.jasmine.server.core.storage.jdbc.DatabaseTableDescription
import com.gridnine.jasmine.server.core.storage.jdbc.JdbcCollectionType
import com.gridnine.jasmine.server.core.storage.jdbc.JdbcFieldValue
import com.gridnine.jasmine.server.core.storage.jdbc.JdbcPropertyType
    ...
    internal fun prepareWherePart(crits: List<SearchCriterion>, freeTextPattern: String?,
        ...
        if (!freeTextPattern.isNullOrEmpty()) {
            for (ptt in freeTextPattern.split(" ")) {
                if (ptt.isNotEmpty()) {
                    criterions.add(SimpleCriterion(aggregatedDataProp.name, SimpleCriterion.Operation.ILIKE, "%${ptt.toLowerCase()}%"))
                }
            }
        }

...
class BasicDomainSerializationTest : CoreTestBase() {
    ...
    fun testSerialization() {
        ...
        JsonSerializer.get().serialize(document, baos, false, true)
        println(baos.toString(Charsets.UTF_8))
        val doc2 = JsonSerializer.get().deserialize<TestDomainDocument>(TestDomainDocument::class.java.name, ByteArrayInputStream(baos.toByteArray()))

...
object UiListHelper {
    fun search(listId:String, criterions: List<BaseWorkspaceCriterion>, filters:List<ListFilter>,columns: List<String>, freeText:String?, offset:Int, limit:Int,sortColumn: String?, sortDesc:Boolean): Pair<Int, List<BaseIdentity>> {
        ...
        val totalCount:Long =  if(DomainMetaRegistry.get().assets.containsKey(listId)){
            Storage.get().searchAssets(ReflectionFactory.get().getClass(listId), projectionQuery)
        } else {
    ...
    fun deleteObjects(references:List<Pair<String,String>>){
        references.forEach {
            if(DomainMetaRegistry.get().assets.containsKey(it.first)){
                Storage.get().deleteAsset(ObjectReference(ReflectionFactory.get().getClass(it.first), it.second, null))
            } else {
                Storage.get().deleteDocument(ObjectReference(ReflectionFactory.get().getClass(it.first), it.second, null))
            }
        }
    }
}
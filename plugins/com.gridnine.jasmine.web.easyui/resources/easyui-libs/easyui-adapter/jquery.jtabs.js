...
(function($){
	...
	function setScrollers(container) {
		var opts = $.data(container, 'jtabs').options;
		...
		if (opts.tools) {
			if (typeof opts.tools == 'string'){
				...
				for(var i=0; i<opts.tools.length; i++){
					...
					tool.menubutton($.extend({}, opts.tools[i], {
						plain: true,
					}));
				}
			}
	...
	function setSize(container, param) {
		var state = $.data(container, 'jtabs');
	...
	function wrapTabs(container) {
		var tabs = $.data(container, 'jtabs').tabs;
	...
	function bindEvents(container){
		...
		$(container).children('div.tabs-header')._unbind()._bind('click', function(e){
			if ($(e.target).hasClass('tabs-scroller-left')){
				$(container).jtabs('scrollBy', -opts.scrollIncrement);
	...
	function createTab(container, options, pp) {
		...
		pp.panel($.extend({}, options, {
			...
			onBeforeOpen: function(){
				...
				var p = $(container).jtabs('getSelected');
				if (p){
					if (p[0] != this){
						$(container).jtabs('unselect', getTabIndex(container, p));
						p = $(container).jtabs('getSelected');
						if (p){
							return false;
						}
					} else {
				...
				if (left < 0 || right > wrap.width()){
					...
					$(container).jtabs('scrollBy', deltaX);
				} else {
					$(container).jtabs('scrollBy', 0);
				}
...
//				}
	...
	function updateTab(container, param){
		...
		var selectHis = $.data(container, 'jtabs').selectHis;
		...
		$.data(container, 'jtabs').options.onUpdate.call(container, opts.title, getTabIndex(container, pp));
	}
...
//		setScrollers(container);
		...
		state.selectHis = his;
		var selected = $(container).jtabs('getSelected');
		if (!selected && his.length){
			index = state.selectHis.pop();
			$(container).jtabs('select', index);
		}
	...
	function showTool(container, visible){
		...
		$(container).jtabs('resize').jtabs('scrollBy', 0);
	}
	
	
	$.fn.jtabs = function(options, param){
		if (typeof options == 'string') {
			return $.fn.jtabs.methods[options](this, param);
		}
		
		options = options || {};
		return this.each(function(){
			var state = $.data(this, 'jtabs');
			if (state) {
				$.extend(state.options, options);
			} else {
				$.data(this, 'jtabs', {
					options: $.extend({},$.fn.jtabs.defaults, $.fn.jtabs.parseOptions(this), options),
					tabs: [],
					selectHis: []
				});
				wrapTabs(this);
	...
	$.fn.jtabs.methods = {
		options: function(jq){
			var cc = jq[0];
			var opts = $.data(cc, 'jtabs').options;
			var s = getSelectedTab(cc);
			opts.selected = s ? getTabIndex(cc, s) : -1;
			return opts;
		},
		tabs: function(jq){
			return $.data(jq[0], 'jtabs').tabs;
		},
		...
		enableTab: function(jq, which){
			return jq.each(function(){
				var opts = $(this).jtabs('getTab', which).panel('options');
				opts.tab.removeClass('tabs-disabled');
				opts.disabled = false;
			});
		},
	...
	$.fn.jtabs.parseOptions = function(target){

...
class EasyUiWebTagbox(private val config: WebSelectConfiguration) : WebSelect,EasyUiComponent {
    ...
    override fun getValues(): List<SelectItemJS> {
        ...
        return (jq.tagbox("getValues") as Array<String>).map{toSelectItem(it)}
    }
    ...
    private fun setValuesInternal() {
        ...
        try {
            jq.tagbox("setValues", values.map {toItem(it)}.toTypedArray())
        } finally {
        ...
        jq.tagbox(object{
            ...
            val loader = {param:dynamic,success:dynamic,_:dynamic ->
                if(config.mode == SelectDataType.LOCAL){
                    success(possibleValues.filter { param.t == null || it.text.toLowerCase().contains((param.t as String).toLowerCase())}.map { toItem(it) }.toTypedArray())
                } else{
                    if(!initialized){
                        success(values.map { toItem(it) }.toTypedArray())
                    } else {
                        launch {
                            val values = this@EasyUiWebTagbox.loader!!.invoke(param.q as String?)
                            success(values.map { toItem(it) }.toTypedArray())
                        }
                    }
                }
            ...
            val onChange = { newValue:String?, _:dynamic ->
                if(!ignoreChangeEvent) {
                    this@EasyUiWebTagbox.changeListener?.let {
                        launch {
